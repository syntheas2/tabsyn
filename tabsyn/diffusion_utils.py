...
def conditional_sample(model, num_samples, dim, condition_mask, condition_values, num_steps=50, device='cuda:0'):
    """
    Conditional sampling with a mask to enforce specific values
    
    Args:
        model: The denoising model
        num_samples: Number of samples to generate
        dim: Dimension of each sample
        condition_mask: Boolean mask indicating which features to condition on (shape: [dim])
        condition_values: Values to enforce for conditioned features (shape: [num_samples, sum(condition_mask)])
    ...
    with torch.no_grad():
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            # Regular sampling step
            x_next = sample_step(model, num_steps, i, t_cur, t_next, x_next)
            
            # Apply condition mask - only at lower noise levels to avoid instability
            if t_next < t_steps[0] * 0.5:  # Apply in the second half of sampling when the data becomes more structured
                # Extract indices of conditions
                cond_indices = torch.where(condition_mask)[0]
                
                # Replace the values at the masked positions with the condition values
                x_next[:, cond_indices] = condition_values
